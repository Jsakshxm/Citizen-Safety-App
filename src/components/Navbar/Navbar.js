...
const { data: { user } } = await supabase.auth.getUser()
  ...
  return (
    <>
      ...
      <div className={`absolute w-full dark:bg-slate-800 z-[10] transition duration-300 lg:hidden ${meuopen?"":"-translate-y-full"}`}>
        <ul className="pt-20 text-center flex flex-col lg:flex text-white ">
          <li className={`hover:text-yellow-300 p-2  transition duration-300 ${meuopen?"":"-translate-x-full"}`}>
            <Link href="/">Home</Link>
          </li>
          <li className={`hover:text-emerald-450 p-2  transition duration-[450ms] ${meuopen?"":"-translate-x-full"}`}>
            {" "}
            <Link href="/UrlSearch">Url search</Link>
          </li>
          <li className={`hover:text-yellow-300 p-2  transition duration-[600ms] ${meuopen?"":"-translate-x-full"}`}>
            <Link href="/SmsAnalysis">SmsAnalysis</Link>
          </li>
          {/* <li className="">
        <Link href="/FraudDetection">FraudDetection</Link>
      </li> */}
          <li className={`hover:text-yellow-300 p-2  transition duration-[750ms] ${meuopen?"":"-translate-x-full"}`}>
            <Link href="/CryptoWalletChecker">Crypto Wallets</Link>
          </li>
          <li className={`hover:text-yellow-300 p-2  transition duration-[900ms] ${meuopen?"":"-translate-x-full"}`}>
            <Link href="/Education">Education</Link>
          </li>
